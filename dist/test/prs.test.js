...
vi.mock('../src/clients/graphql.js', () => {
    return {
        createGraphqlClient: () => async (q, _v) => {
        ...
        createRestClient: () => ({
            async request(route, params) {
                if (route.startsWith('GET /repos/{owner}/{repo}/pulls/{number}/files')) {
                    return { data: [{ filename: 'a', status: 'modified', additions: 1, deletions: 0, changes: 1, sha: 's', patch: '---' }], headers: { 'X-RateLimit-Remaining': '4999', 'X-RateLimit-Used': '1' } };
                }
                if (route === 'GET /repos/{owner}/{repo}/pulls/{number}') {
                    if ((params.headers?.Accept || '').includes('diff'))
                        return { data: 'diff', headers: {} };
                    if ((params.headers?.Accept || '').includes('patch'))
                        return { data: 'patch', headers: {} };
                }
                return { data: {}, headers: {} };
            },
        }),
...
import { listPullRequests, getPullRequest, getPrStatusSummary, listPrReviewCommentsPlain, listPrFilesLight, getPrDiff, getPrPatch, } from '../src/handlers/prs.js';
beforeEach(() => {
...
describe('prs', () => {
    it('listPullRequests returns items', async () => {
        const r = await listPullRequests({ owner: 'o', repo: 'r', include_author: true });
        expect(r.items[0].author_login).toBe('a');
    });
    it('getPullRequest returns minimal fields', async () => {
        const r = await getPullRequest({ owner: 'o', repo: 'r', number: 1, include_author: true });
        expect(r.item.author_login).toBe('a');
    });
    it('getPrStatusSummary maps union correctly', async () => {
        const r = await getPrStatusSummary({ owner: 'o', repo: 'r', number: 1, include_failing_contexts: true });
    ...
    it('listPrFilesLight omits patch by default', async () => {
        const r = await listPrFilesLight({ owner: 'o', repo: 'r', number: 1 });
        expect(r.items[0].patch).toBeUndefined();
    });
    it('getPrDiff and getPrPatch use media types', async () => {
        const d = await getPrDiff({ owner: 'o', repo: 'r', number: 1 });
        const p = await getPrPatch({ owner: 'o', repo: 'r', number: 1 });
        expect(d.diff).toBe('diff');
        expect(p.patch).toBe('patch');
    });
    it('listPrReviewCommentsPlain include_location', async () => {
        const r = await listPrReviewCommentsPlain({ owner: 'o', repo: 'r', number: 1, include_location: true });
        expect(r.items[0].path).toBe('a.ts');
    });
});
//# sourceMappingURL=prs.test.js.map