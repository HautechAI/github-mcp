...
import type { Meta } from '../utils/types.js';
import type { GetIssueInput, ListIssueCommentsInput, ListIssuesInput } from '../schemas/issues.js';
...
export async function listIssues(input: ListIssuesInput) {
  try {
    ...
    const filterBy: any = {};
    if (input.labels && input.labels.length) filterBy.labels = input.labels;
    if (input.creator) filterBy.createdBy = input.creator;
    if (input.assignee) filterBy.assignee = input.assignee;
    if (input.mentions) filterBy.mentioned = input.mentions;
    if (input.since) filterBy.since = input.since;
    const query = /* GraphQL */ `
...
export async function listIssueCommentsPlain(input: ListIssueCommentsInput) {
  try {
    const graphql = createGraphqlClient();
    const query = /* GraphQL */ `
