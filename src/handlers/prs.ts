...
import type {
  GetPrDiffInput,
  GetPrPatchInput,
  GetPrStatusSummaryInput,
  GetPullRequestInput,
  ListPrCommentsInput,
  ListPrCommitsInput,
  ListPrFilesInput,
  ListPrReviewCommentsInput,
  ListPrReviewsInput,
...
export async function getPullRequest(input: GetPullRequestInput) {
  try {
    ...
    `;
    const data: any = await graphql(query, { owner: input.owner, repo: input.repo, number: input.number });
...
export async function getPrStatusSummary(input: GetPrStatusSummaryInput) {
  try {
    const graphql = createGraphqlClient();
    const query = /* GraphQL */ `
    ...
    let success = 0,
      pending = 0,
      failure = 0;
    const failing_contexts: string[] = [];
    for (const c of contexts) {
      if (c.__typename === 'CheckRun') {
        const concl = (c.conclusion || '').toUpperCase();
        if (concl === 'SUCCESS' || concl === 'NEUTRAL' || concl === 'SKIPPED') success++;
        else if (concl === 'PENDING' || concl === 'QUEUED' || concl === 'IN_PROGRESS') pending++;
        else {
          failure++;
          if (input.include_failing_contexts) failing_contexts.push(c.name);
        }
        ...
        if (state === 'SUCCESS') success++;
          ...
          failure++;
          if (input.include_failing_contexts) failing_contexts.push(c.context);
        }
      }
    }
...
export async function listPrCommentsPlain(input: ListPrCommentsInput) {
  try {
    const graphql = createGraphqlClient();
    const query = /* GraphQL */ `
...
export async function listPrReviewCommentsPlain(input: ListPrReviewCommentsInput) {
  try {
    const graphql = createGraphqlClient();
    const query = /* GraphQL */ `
...
export async function listPrReviewsLight(input: ListPrReviewsInput) {
  try {
    const graphql = createGraphqlClient();
    const query = /* GraphQL */ `
...
export async function listPrCommitsLight(input: ListPrCommitsInput) {
  try {
    const graphql = createGraphqlClient();
    const query = /* GraphQL */ `
...
export async function listPrFilesLight(input: ListPrFilesInput) {
  try {
    ...
    const items = (res.data as any[]).map((f: any) => ({
      filename: f.filename,
      status: f.status,
...
export async function getPrDiff(input: GetPrDiffInput) {
  try {
    ...
    return { diff: String(res.data), meta: { rate: parseRestRateHeaders(res.headers as any) } };
  } catch (err) {
    return mapError(err);
  }
}

export async function getPrPatch(input: GetPrPatchInput) {
  try {
    const rest = createRestClient();
    const res = await rest.request('GET /repos/{owner}/{repo}/pulls/{number}', {
      owner: input.owner,
    ...
    return { patch: String(res.data), meta: { rate: parseRestRateHeaders(res.headers as any) } };
  } catch (err) {
    return mapError(err);
  }
}
