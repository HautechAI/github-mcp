...
fn get_workflow_job_logs_redirect_zip_tail_and_timestamps() -> anyhow::Result<()> {
    ...
    let _m1 = server.mock(|when, then| { when.method(GET).path("/repos/o/r/actions/jobs/42/logs"); then.status(302).header("location", redirect_url.as_str()); });
    let _m2 = server.mock(|when, then| { when.method(GET).path("/tmp/log.zip"); then.status(200).body(zip_bytes.clone()); });
...
fn rerun_and_cancel_endpoints() -> anyhow::Result<()> {
    let server = MockServer::start();
    let _rerun = server.mock(|when, then| { when.method(POST).path("/repos/o/r/actions/runs/100/rerun"); then.status(202); });
    let _rerun_failed = server.mock(|when, then| { when.method(POST).path("/repos/o/r/actions/runs/200/rerun-failed-jobs"); then.status(202); });
    let _cancel = server.mock(|when, then| { when.method(POST).path("/repos/o/r/actions/runs/300/cancel"); then.status(202); });
    ...
    let out1 = run_with_env(&req1, &[("GITHUB_TOKEN","t"),("GITHUB_API_URL", server.base_url().as_str())])?;
    let out2 = run_with_env(&req2, &[("GITHUB_TOKEN","t"),("GITHUB_API_URL", server.base_url().as_str())])?;
    let out3 = run_with_env(&req3, &[("GITHUB_TOKEN","t"),("GITHUB_API_URL", server.base_url().as_str())])?;
