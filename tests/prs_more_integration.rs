...
use httpmock::{Method::POST, Method::GET, MockServer};
...
fn list_pr_comments_plain_happy() -> anyhow::Result<()> {
    ...
    let _m = server.mock(|when, then| { when.method(POST).path("/graphql"); then.status(200).json_body(body); });
...
fn get_pr_diff_and_patch_rest_headers() -> anyhow::Result<()> {
    let server = MockServer::start();
    let _m1 = server.mock(|when, then| { when.method(GET).path("/repos/o/r/pulls/1"); then.status(200).header("x-ratelimit-remaining","4999").header("x-ratelimit-used","1").header("x-ratelimit-reset","0").body("diff-data"); });
    ...
    let _m2 = server.mock(|when, then| { when.method(GET).path("/repos/o/r/pulls/2"); then.status(200).body("patch-data"); });
